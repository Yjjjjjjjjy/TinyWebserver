...
void http_conn::init()
{
    ...
    byte_to_send = 0;
    bytes_have_send = 0;
    m_check_state = CHECK_STATE_REQUESTILINE;
    m_linger = false;
    m_method = GET;
    m_url = 0;
    m_version = 0;
    ...
    if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))
    {
        ...
        if (*(p + 1) == '3')
        {
            ...
            if (users.find(name) == users.end())
            {
                ...
                strcpy(m_url, "/resisterError.html");
        }
        else if (*(p + 1) == '2')
        {
...
{
    return add_response("Connection:%s\r\n", (m_linger == true) ? "keep-active" : "close");
}
